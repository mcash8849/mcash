...
unsigned int nStakeMinAge = 6 * nOneHour; // 6  hours  as zero time weight
unsigned int nStakeMaxAge = 1.5 * 24 * nOneHour; // 2 days as full weight
...
const string strMessageMagic = "MCASH Signed Message:\n";
...
{
    int64_t nRewardCoinYear = 0.0005 * CENT;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in MCASH
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // MCASH: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "MCASH", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "mcash";
        CTransaction txNew;
        txNew.nTime = 1506630685;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1506630689;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3263107; // !fTestNet ? 1575379 : 46534;
