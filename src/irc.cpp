...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MCASHTEST2\r");
            Send(hSocket, "WHO #MCASHTEST2\r");
        } else {
            // randomly join #MCASH00-#MCASH05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MCASH%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MCASH%02d\r", channel_number).c_str());
        }
