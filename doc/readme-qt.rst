...
Alternatively, install Qt Creator and open the `MCASH-qt.pro` file.

An executable named `MCASH-qt` will be built.
...
A warning for people using the *static binary* version of MCASH on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of MCASH is linked against libdb5.3.
...
installed causes MCASH-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
